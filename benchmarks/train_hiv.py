...
def get_parser():
    parser = argparse.ArgumentParser(description='Training script for the Hypercomplex MPNN on HIV data')
    ...
    parser.add_argument('--save_dir', action='store', dest='save_dir', default="hiv/`save_dir`", type=str,
                        help="Where the results should be saved. Defaults to hiv/`save_dir`")
    parser.add_argument('--n_runs', action='store', dest='n_runs', default=10, type=int,
                        help="How many runs should be done. Defaults to 10.")
    ...
    parser.add_argument('--input_embed_dim', action='store', type=int, default=200,
                        help="(Embedding) Dimension for the input features. Defaults to 200")
    ...
    parser.add_argument('--mp_units', help='Number of hidden units for each message passing layer.'
                                           'Defaults to "200,200"', type=str, default="200,200")
    ...
    parser.add_argument('--dropout_mpnn', help='Dropouts for message passing layers.'
                                               'Defaults to "0.3,0.1" ', type=str, default="0.3,0.1")
    ...
    parser.add_argument('--d_units', help='Number of hidden units in the Downstream Network.'
                                          'Defaults to "128,32" ', type=str, default="128,32")
    parser.add_argument('--d_bn', help='Which batch-norm should be used for the downstream network'
                        ...
                        choices=["None", "naive-batch-norm", "q-batch-norm", "naive-naive-batch-norm"])
    parser.add_argument('--dropout_dn', type=str, default="0.3,0.1",
                        help="Dropout rate for the downstream network. Defaults to '0.3,0.1'.")
    ...
    parser.add_argument('--aggr_msg', help='Which scheme to use for aggregating the messages m_vw.'
                                           ' Defaults to softmax.',
                        type=str, default="softmax", choices=["sum", "mean", "min", "max", "softmax", "pna"])
    parser.add_argument('--aggr_node', help='Which scheme to use for aggregating the node embeddings.'
                                            ' Defaults to softmax.',
                        type=str, default="softmax", choices=["sum", "mean", "min", "max", "softmax", "pna"])
    ...
    parser.add_argument('--epochs', type=int, default=50,
                        help="Number of epochs to train. Defaults to 50.")
...
def main():
    ...
    # change to False. Need to further change API to make it work
    unique_phm = False
