...
def get_parser():
    parser = argparse.ArgumentParser(description='Training script for the hypercomplex MPNN on PPA data')
    ...
    parser.add_argument('--save_dir', action='store', dest='save_dir', default="ppa/`save_dir`", type=str,
                        help="Where the results should be saved. Defaults to ppa/`save_dir`")
    ...
    parser.add_argument('--phm_dim', action='store', type=int, default=4,
                        help="Dimension of hypercomplex components. Defaults to 4.")
    ...
    parser.add_argument('--input_embed_dim', action='store', type=int, default=500,
                        help="(Embedding) Dimension for the input features. Defaults to 500")
    ...
    parser.add_argument('--mp_units', help='Number of hidden units for each message passing layer.'
                                           'Defaults to "500,500,500,500,500"',
                        type=str, default="500,500,500,500,500")
    ...
    parser.add_argument('--d_units', help='Number of hidden units in the Downstream Network.'
                                          'Defaults to "512,256" ', type=str, default="512,256")
    parser.add_argument('--d_bn', help='Which batch-norm should be used for the downstream network'
                        ...
                        choices=["None", "naive-batch-norm", "q-batch-norm",
                                 "naive-naive-batch-norm"])
    parser.add_argument('--dropout_dn', type=str, default="0.2,0.1",
                        help="Dropout rate for the downstream network. Defaults to '0.2,0.1' .")
    ...
    parser.add_argument('--epochs', type=int, default=200,
                        help="Number of epochs to train. Defaults to 200.")
    parser.add_argument('--lr', action='store', type=float, default=0.001,
                        help="Learning rate for AdamW Optimizer. Defaults to 0.001")
    parser.add_argument('--patience', type=int, default=10,
                        help="Patience for learning rate scheduler. Defaults to 10.")
    parser.add_argument('--factor', type=float, default=0.75,
                        help="Multiplicative learning rate factor. Defaults to 0.75.")
    parser.add_argument('--weightdecay', action='store', type=float, default=0.001,
                        help="weight decay for weight matrices in PHM-Layers. Defaults to 0.001 ")
    ...
    parser.add_argument('--grad_clipping', action='store', type=float, default=2.0,
                        help="Gradient clipping. Defaults to max-norm 2.0 - If no gradient clipping should be used,"
                             "insert 0.0")
    parser.add_argument('--node_pre_trafo', action='store', type=str, default="add-zeros",
                        choices=["add-zeros", "aggr-add", "aggr-mean", "aggr-min", "aggr-max"],
                        help="How to pre-transform the node features, as the PPA dataset does consists of "
                             "node features. Options are to add-zeros or aggregate the incoming edge-features through"
                             "the `add`, `mean`, `min` or `max` value. Defaults to 'aggr-add'.")
    ...
    for i, data in enumerate(loader):
        try:
            if data.x.shape[0] == 1 or data.batch[-1] == 0:
                ...
                optimizer.zero_grad()
                logits = model(data)
                loss = multicls_criterion(input=logits, target=data.y.view(-1,))
                ...
                y_true_list.append(data.y.view(-1, 1).detach().cpu())
                y_pred_list.append(torch.argmax(logits.detach(), dim=1).view(-1, 1).cpu())
...
def test_validate(model, device, transform, loader, evaluator):
    ...
    for i, data in enumerate(loader):
        if data.x.shape[0] == 1:
            ...
            loss = multicls_criterion(input=logits, target=data.y.view(-1, ))
            total_loss += loss.item() * data.num_graphs
            y_true_list[i] = data.y.view(-1, 1).detach().cpu()
            y_pred_list[i] = torch.argmax(logits.detach(), dim=1).view(-1, 1).cpu()
...
def do_run(i, model, args, transform, train_loader, valid_loader, test_loader, device, evaluator):
    ...
    if args.weightdecay > 0.0:
        ...
        total_params_splitted = sum([sum([p.numel() for p in pl["params"]]) for pl in params])
...
def main():
    ...
    # data
    dname = "ogbg-ppa"
    if args.node_pre_trafo == "add-zeros":
        transform = add_zeros
        atom_input_dims = [1]
    else:
        transform = partial(extract_node_feature, reduce=args.node_pre_trafo.split("-")[-1])
        atom_input_dims = 7

    dataset = PygGraphPropPredDataset(name=dname, root="dataset", transform=transform)
    ...
    if args.aggr_msg == "pna" or args.aggr_node == "pna":
        ...
        deg = torch.zeros(300, dtype=torch.long)
    ...
    if args.type == "undirectional-quaternion-sc-add":
        ...
        model = UQ_SC_ADD(atom_input_dims=atom_input_dims,
                          ...
                          target_dim=dataset.num_classes,
        ...
        model = UQ_SC_CAT(atom_input_dims=atom_input_dims,
                          atom_encoded_dim=args.input_embed_dim,
                          bond_input_dims=7,
                          naive_encoder=naive_encoder,
                          mp_layers=mp_layers, dropout_mpnn=mp_dropout,
        ...
        model = UPH_SC_ADD(phm_dim=phm_dim, learn_phm=learn_phm,
                           atom_input_dims=atom_input_dims,
                           atom_encoded_dim=args.input_embed_dim,
                           bond_input_dims=7,
                           naive_encoder=naive_encoder,
                           mp_layers=mp_layers, dropout_mpnn=mp_dropout,
        ...
        model = UPH_SC_CAT(phm_dim=phm_dim, learn_phm=learn_phm,
                           atom_input_dims=atom_input_dims,
                           atom_encoded_dim=args.input_embed_dim,
                           bond_input_dims=7,
                           naive_encoder=naive_encoder,
