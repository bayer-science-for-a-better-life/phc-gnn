...
def get_parser():
    parser = argparse.ArgumentParser(description='Training script for the hypercomplex MPNN on CIFAR10 data')
    ...
    parser.add_argument('--save_dir', action='store', dest='save_dir', default="cifar10/`save_dir`", type=str,
                        help="Where the results should be saved. Defaults to cifar10/`save_dir`")
...
def main():
    ...
    train_data = GNNBenchmarkDataset(path, name="CIFAR10", split='train', transform=transform)
    valid_data = GNNBenchmarkDataset(path, name="CIFAR10", split='val', transform=transform)
    test_data = GNNBenchmarkDataset(path, name="CIFAR10", split='test', transform=transform)
    evaluator = Evaluator()
