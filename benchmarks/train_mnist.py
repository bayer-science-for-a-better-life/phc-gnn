...
def get_parser():
    parser = argparse.ArgumentParser(description='Training script for the hypercomplex MPNN on MNIST data')
    ...
    parser.add_argument('--batch_size', action='store', dest='batch_size', default=128, type=int,
                        help="Batch size for training. Defaults to 128.")
    parser.add_argument('--save_dir', action='store', dest='save_dir', default="mnist/`save_dir`", type=str,
                        help="Where the results should be saved. Defaults to mnist/`save_dir`")
    parser.add_argument('--n_runs', action='store', dest='n_runs', default=4, type=int,
                        help="How many runs should be done. Defaults to 4.")
    ...
    parser.add_argument('--input_embed_dim', action='store', type=int, default=224,
                        help="(Embedding) Dimension for the input features. Defaults to 224")
    ...
    parser.add_argument('--mp_units', help='Number of hidden units for each message passing layer.'
                                           'Defaults to "224,224,224,224"', type=str, default="224,224,224,224")
    ...
    parser.add_argument('--dropout_mpnn', help='Dropouts for message passing layers.'
                                               'Defaults to "0.1,0.1,0.1,0.1" ', type=str, default="0.1,0.1,0.1,0.1")
    ...
    parser.add_argument('--d_units', help='Number of hidden units in the Downstream Network.'
                                          'Defaults to "256,128" ', type=str, default="256,128")
    ...
    # Benchmarking-GNNs arguments
    parser.add_argument('--min_lr', action='store', type=float, default=1e-6,
                        help="Minimum learning rate to execute the script. Default: 1e-6")
    parser.add_argument('--max_time', action='store', type=float, default=72,
                        help="Maximum running time of script in hours. Default: 72")
    parser.add_argument('--epochs', type=int, default=1000,
                        help="Number of epochs to train. Defaults to 1000.")
    ...
    parser.add_argument('--sc_type', action="store", type=str, default="last",
                        choices=["first", "last"],
...
class Evaluator(object):
    ...
    def _eval_acc(self, y_true, y_pred):
        y_pred_pred = y_pred.max(dim=1)[1]
        acc = (y_true == y_pred_pred).to(torch.float).mean().item()
        return {'acc': acc}

    def eval(self, dict_data: dict) -> dict:
        assert "y_pred" in list(dict_data.keys()) and "y_true" in list(dict_data.keys())
        return self._eval_acc(y_true=dict_data["y_true"], y_pred=dict_data["y_pred"])


def train(epoch, model, device, transform, loader, optimizer, evaluator, kwargs: dict):
    model = model.train()
    total_loss = 0.0
    y_true_list = list()
    y_pred_list = list()
    for i, data in enumerate(loader):
        ...
        except RuntimeError:  # for directional models bc. of torch_scatter
            logging.info(f"runtime error at step {i}")
            continue
        if i % 200 == 0 and i != 0:
...
def do_run(i, model, args, transform, train_loader, valid_loader, test_loader, device, evaluator, t0):
    ...
    try:
        for epoch in range(1, args.epochs + 1):
            ...
            if time.time() - t0 > args.max_time * 3600:
                logging.info('-' * 89)
                logging.info("Max_time for training elapsed {:.2f} hours, so stopping".format(args.max_time))
                break
...
def main():
    ...
    train_data = GNNBenchmarkDataset(path, name="MNIST", split='train', transform=transform)
    valid_data = GNNBenchmarkDataset(path, name="MNIST", split='val', transform=transform)
    test_data = GNNBenchmarkDataset(path, name="MNIST", split='test', transform=transform)
    evaluator = Evaluator()
