...
def get_parser():
    parser = argparse.ArgumentParser(description='Training script for the hypercomplex MPNN on ZINC data')
    ...
    parser.add_argument('--save_dir', action='store', dest='save_dir', default="zinc/`save_dir`", type=str,
                        help="Where the results should be saved. Defaults to zinc/`save_dir`")
    parser.add_argument('--subset_data', action='store', dest='subset_data', default="True", type=str,
                        help="If the subset for training should be used only. Defaults to True.")
    ...
    parser.add_argument('--input_embed_dim', action='store', type=int, default=180,
                        help="(Embedding) Dimension for the input features. Defaults to 180")
    ...
    parser.add_argument('--mp_units', help='Number of hidden units for each message passing layer.'
                                           'Defaults to "180,180,180,180"', type=str, default="180,180,180,180")
    ...
    parser.add_argument('--dropout_mpnn', help='Dropouts for message passing layers.'
                                               'Defaults to "0,0,0,0" ', type=str, default="0,0,0,0")
    ...
    parser.add_argument('--d_units', help='Number of hidden units in the Downstream Network.'
                                          'Defaults to "180,80" ', type=str, default="180,80")
    ...
    parser.add_argument('--weightdecay', action='store', type=float, default=0.01,
                        help="weight decay for weight matrices in PHM-Layers. Defaults to 0.01 ")
...
class Evaluator(object):
    ...
    def eval(self, dict_data: dict) -> dict:
        ...
        MAE = F.l1_loss(dict_data["y_pred"].squeeze(), dict_data["y_true"].squeeze()).item()

        return {self.eval_metric: MAE}
...
def train(epoch, model, device, transform, loader, optimizer, evaluator, kwargs: dict):
    ...
    for i, data in enumerate(loader):
        ...
        optimizer.zero_grad()
        logits = model(data)
        #loss = loss_func(logits.reshape(data.y.size(0), 1), data.y.reshape(data.y.size(0), 1))
        loss = (logits.squeeze() - data.y).abs().mean()
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            train_metrics = train(epoch=epoch, model=model, device=device, transform=transform,
                                  ...
                                  kwargs={"run": i, "nruns": args.n_runs, "grad_clipping": args.grad_clipping,
                                          "lr": lr,
                                          "weight_decay": args.weightdecay, "weight_decay2": args.weightdecay2,
